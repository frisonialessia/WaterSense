...
export function HourlyPrices({ th, tr, prices, parcels }: { th: Theme; tr: (s: string, t: string) => string; prices: number[]; parcels: Parcel[] }) {
  ...
  const cheapest = useMemo(() => prices.indexOf(Math.min(...prices)), [prices]);
  ...
  useEffect(() => {
    ...
    const input: DecisionInput = {
      ...
      currentTariff: { pricePerKwh: prices[nowHour] ?? 1.8, startsInHours: 0, durationHours: 4 },
      nextLowTariff: { pricePerKwh: prices[cheapest] ?? 0.6, startsInHours: hoursUntilCheap, durationHours: 4 },
    ...
    <div style={{ padding: `${space.xs}px ${space.xl}px ${space.lg}px` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: space.md }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div className="mono" style={{ fontSize: fz.lg, fontWeight: 700, color: hour === cheapest ? C.emerald : C.glacier }}>${prices[hour].toFixed(2)}</div>
      ...
      <p style={{ fontSize: fz.micro, color: th.mute, marginTop: space.sm }}>
        {tr("Recomendación calculada por el motor de decisión (/api/decision) con datos simulados.", "Decisión vía /api/decision · entradas representativas simuladas.")}
      </p>
    </div>
  );
}
